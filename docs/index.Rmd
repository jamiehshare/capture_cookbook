...
```{r, class.source = 'fold-show', eval = FALSE, tidy=TRUE, tidy_source(width.cutoff = 60)}
...
df %>% 
  ...
  labs(x = "Date",
       y = "Number of posts",
       title = "Bar chart showing daily trend") +
...
This limitation can be somewhat mitigated by directly adding values onto the plot, but we will delve into that aspect [later in this document](#values)
...
plot_group_term_network_jh <- function(data, group_var = brand, text_var = message, n_terms = 20,
  ...
  nodes <- nodes %>%
    ...
    dplyr::mutate(n = ifelse(is.na(n), 1, n),
                  ...
                  fontface = ifelse(node_name %in% groups, "bold", "plain"))
...
```{r group-term-network, fig.cap = "A network showing overlapping and distinct terms across different categories"}
set.seed(1234)
example %>% 
  ParseR::clean_text(text_var = message) %>% 
  dplyr::mutate(message = tm::removeWords(x = message, c(tm::stopwords(kind = "SMART"), 
...
```{r gg-heatmap-4, fig.cap = 'The sum of the rows = 100% in this case', out.width="60%"}
x <- LETTERS[1:6]
y <- paste0("Topic ", seq(1, 5))
...
data %>% 
  ...
  scale_fill_viridis_c(direction = -1,
                       labels = percent) +
  geom_text(aes(label = round(z_prop, 2),
            colour = ifelse(z_prop >0.4, "white", "black"))) +
