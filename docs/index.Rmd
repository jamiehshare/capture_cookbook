...
capture_theme <- function(base_size = 11, base_family = "GT Walsheim Pro",
...
In line with this, this document will not teach you R or the technical aspects of creating these plots. Whilst there is some code provided for some plots- showing 'how the plots are made', there is minimum explanation of the code in the hope of avoiding too much boilerplate and to try and make this document as coding language/software-agnostic as possible. Instead, its primary goal is to help you grasp the purpose and appropriate usage of visualisations in our work. By doing so, it empowers everyone to confidently explain the key insights, strengths, and weaknesses conveyed by different types of plots. Once the building blocks of this are in place, it is much easier to apply this knowledge to different coding languages and software. *Note that if you do look at the code for the plots, there is a function called `capture_theme()` that is used. This is not yet available for wider use, but will eventually appear in the `HelpR` package to enable consistent 'Capture aesthetics' for each plot*
...
```{r gg-treemap, fig.caption = "A treemap showing part-to-whole relationship of social media mentions within the whole conversation using the size and colour of rectangles to encode values", fig.width=6}
...
```{r gg-treemap-2, fig.caption = "A treemap showing part-to-whole relationship of social media mentions within the whole conversation using the size and colour of rectangles to encode values", fig.width=6}
treemap <- read_rds("~/Documents/R/capture_cookbook/data/treemap.rds")
...
Sometimes we will want to understand the distribution of a **continuous variable** in our data to gain insights. For instance, if we have calculated valence using the ParseR package, we might want to examine the distribution. Although these types of visualisations are typically used for exploratory data analysis (EDA) rather than client presentations, it is still an extremely powerful and useful string to have in your data exploration bow.
...
## Visualise most frequent terms (Bigram network)
...
## Differences in language between categories (WLO)
...
## Visualising complex conversational landscapes (UMAP)
...
*Please note that this is a very high level overview of UMAP. As explained previously, the purpose of this Cookbook is not to get a thorough understanding of statistics and Data Science, but rather empower solid data visualisation principles and understanding from an interpretation perspective*
...
# Generate a color palette using viridis_pal()
color_palette <- viridis_pal()(num_groups)

plots <- purrr::map2(group_names, color_palette,
  ~ umap_tib %>%
    ...
    geom_point(data = . %>% filter(highlight == "A"),
               ...
               stroke = 0,
               size = 2) +
    scale_colour_manual(values = .y) +  # Use manual color palette
    coord_fixed() +
    theme_bw() +
    ggplot2::labs(
      title = paste0(.x),
...
```{r, eval = FALSE, echo = TRUE}
df %>% 
  ggplot(aes(x_axis_category, y_axis_category, fill = fill_variable)) + 
  geom_tile() +
  labs(x = "Brand",
       y = NULL,
...
data %>% 
  ...
  theme(panel.grid = element_blank(),
        ...
        axis.line = element_blank())
```
 **Note, for a bunch of plots I've also created the same plot but with a more 'obvious' sequential colour palette. Just testing to see whether some plots need this "Yellow, Green, Blue" palette for a clearer interpretation of both saturation and hue changing with values**
```{r gg-heatmap-3, fig.cap = 'Example heatmap showing branded conversation within different topics'}
x <- LETTERS[1:6]
y <- paste0("Topic ", seq(1, 5))
...
```{r gg-heatmap-2-v2, fig.cap = 'Example heatmap showing hourly social media brand mentions for each day of the week'}
x <- 1:10000
days <- factor(c("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"), 
               levels = rev(c("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun")), 
               ordered = TRUE)
...
```{r sequential-col-2, fig.cap = 'Example sequential colours'}
seq_bin <- data.frame(colours = seq(1, 7),
                      values = seq(1, 7)) %>% 
  ggplot(aes(x = colours, y = 0, fill = factor(values))) +
  geom_tile() +
...
```{r, col-ex-v2, fig.height = 3, fig.cap = 'We *can* use a sequential palette when we have already ordered categories based on the value of interest'}
qual_bar <- tribble(
  ~colA, ~n, 
  "A", 30,
  "B", 20,
...
```
**NTS, TO DECIDE WHETHER TO KEEP WITH VIRIDIS DEFAULT OR CHANGE TO A 'NICER' SEQUENTIAL PALETTE THAT ALSO CHANGES SATURATION I.E BELOW**
...
```{r, eval = FALSE, echo = TRUE}
## This is the default aesthetics when using the capture_theme() function
... +
  capture_theme()
```
...
## No border, but axis lines included is the default aesthetics when using the capture_theme() function
... +
  capture_theme()
...
```{r gg-highlight-bad, fig.height=3}
set.seed(1234)
line_busy <- example %>% 
  mutate(created_time = floor_date(created_time,
                                   unit = "day")) %>% 
...
```{r gg-highlight-good, fig.height=3}
set.seed(1234)
line_nice <- example %>% 
  mutate(created_time = floor_date(created_time,
                                   unit = "day")) %>% 
